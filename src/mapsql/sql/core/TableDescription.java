...
public class TableDescription {
	 ...
	 * @param cols - the columns name user input
	 * @throws SQLException 
	 */
	public void checkForNotNulls(String[] cols) throws SQLException {
// 		missed - true(if columns have been missed) 
//		false(if columns have not missed)	
		boolean missed = true;
		
		for(Field field : fields){			
			if(field.isNotNull()){
				for(String col: cols){
//					check whether not null columns have been filled in data
					if(field.name().equalsIgnoreCase(col)){
						missed = false;
						break;
					}
				}
