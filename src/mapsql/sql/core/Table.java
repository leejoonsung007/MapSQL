...
//				check whether the row exists
				if(!row.data.isEmpty()) {
					for (int a = 0; a < columns.length; a++) {
						// update the value;
						row.data.put(columns[a], values[a]);
					}
				}
				else throw new SQLException("No record");
			}
		}
	}
...
//		get the first position
		Position<Row> curren_position = rows.first();
		
		for (Row row: rows) {
			if (row.satisfies(where, description)) {
				rows.remove(curren_position);
				break;
			}
//			move to next position until row satisfies the where condition
			else curren_position = rows.next(curren_position);
		}	
	}
}
